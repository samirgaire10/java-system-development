...
public class L0517_pr5 extends BaseFrame {

    Connection myCon = null;
    JButton btnDraw = new JButton("検　索");
    JTextArea tarScreen = new JTextArea();
    JTextField txtValue = new JTextField();
    JLabel lbl1 = new JLabel("ACCESS_DB「sample1.accdb」の検索");
    JLabel lbl2 = new JLabel("　CORPORATION_DATA(C_ ID / NAME /C_ AGE / C_TEL)表　where句から入力↓");
    JPanel panel = new JPanel();;

    //▼ mainメソッド（Javaアプリケーション実行時最初に呼ばれるメソッド）の定義
    //▼ mainメソッドの説明は、前期使用テキストP.40
    public static void main(String args [ ]) {
      L0517_pr5 myAppli = new L0517_pr5("Search records");
      myAppli.setSize(600,320);
    ...
    public L0517_pr5(String title) {
      ...
      btnDraw.addActionListener(new ActionListener() {
        ...
        String user = "";
              ...
              //▼▼SQLステートメント（検索）を実行 ③
              ResultSet myRs = mySt.executeQuery("SELECT * FROM CORPORATION_DATA "

              + txtValue.getText());
              //▼実行結果をレコード移動しながら存在するか評価する
              while (myRs.next()){
                  //▼検索でヒットしたid列値を取得
                  long id = myRs.getLong("C_ID") ;
                  //▼検索でヒットしたname列値を取得
                  String name = myRs.getString("C_NAME") ;
                  //▼検索でヒットしたage列値を取得
                  String address = myRs.getString("C_ADDRESS") ;
                  //▼検索でヒットしたtel列値を取得
                  String tel = myRs.getString("C_TEL") ;
                  //▼取得した各列値をtarScreenへ追記
                  tarScreen.append(id + " / " + name + " / " + address + " / " + tel + "\n");
